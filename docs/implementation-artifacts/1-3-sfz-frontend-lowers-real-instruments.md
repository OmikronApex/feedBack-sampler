...
- 2026-07-19: CI green on all four jobs (linux, macos, windows, linux-asan-ubsan) — run 29685980835. Three platform-specific fixes were needed, one per platform: (1) pin `CMAKE_CXX_STANDARD 17` as a FORCE cache entry so sfizz's own `set(... 14 CACHE)` can't drop GCC/Clang builds to C++14 (`fs` alias split std::filesystem vs ghc::filesystem broke linking on linux/mac); (2) patch sfizz's hardcoded macOS deployment target 10.9 → 10.15 so ghc/fs_std.hpp maps to std::filesystem on mac; (3) unified-diff blank context lines + `cmake/patches/*.patch eol=lf` so Windows autocrlf can't corrupt the sfizz patch. Local pre-push verification workflow established: Windows natively + both Linux legs replicated in Docker (ubuntu:24.04, named volume `fbsampler-ci-build`).
